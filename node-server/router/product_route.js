...
router.post('/api/saveproduct',(req, res)=>{
    ...
    productDataObject.save()
        ...
        .catch((err)=>{
            console.error("Error while saving product:", err); // Log the error to the console
            res.status(500).send("Error in product saving: " + err.message); // Send a 500 Internal Server Error response with the error message
        });
});
